...
const UserState = (props) => {
  ...
  const userUpdate = async (edata) => {
    ...
    const response = await fetch(`${URL}/api/user/update/${id}`, {
      ...
      body: JSON.stringify({
        first_name: edata.efirst_name,
        last_name: edata.elast_name,
        gender: edata.egender,
        avatar: edata.eavatar,
        domain: edata.edomain,
        available: edata.eavailable,
      }), // body data type must match "Content-Type" header
    });
    const Json = await response.json();
    if (Json.success) {
  ...
  const searchByName = async (query) => {
    ...
    const response = await fetch(
      `${URL}/api/query/searchByName?search=${query}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
  ...
  const getFilterData = async (data) => {
    ...
    const response = await fetch(
      `${URL}/api/query/searchByFilter?domain=${domain}&gender=${gender}&available=${available}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
  ...
  const createTeam = (users) => {
    ...
    setSelectedUsers([
      ...selectedUsers,
      { id: id, first_name: first_name, last_name: last_name, domain: domain },
    ]);
  };
